...
export default function RadixSortGamePage({ mode, onExit }) {
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="flex-grow bg-white rounded-3xl shadow-xl border border-slate-200 p-8">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {data.map((val, idx) => (
              <div key={`${val}-${idx}`} className={`px-3 py-1 rounded-lg border text-sm font-bold ${idx === scanIdx ? 'bg-amber-50 border-amber-400 text-amber-700' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>{val}</div>
            ))}
          </div>
